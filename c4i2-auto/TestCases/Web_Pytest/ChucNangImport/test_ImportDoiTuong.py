...
def browser():
    ...
    page_base.click_obj(Obj_VuViec.iconVuViec)
    page_base.click_obj(Obj_VuViec.tabNhapLieu)
    time.sleep(2)
...
@allure.feature("Nhập liệu Đối tượng")
@pytest.mark.Import
@pytest.mark.ImportObj
class Test_Import_DoiTuong:
    pageBase = PageBase(browser)
    pathDataTestImport = pageBase.load_path_data_file_from_path("Datas_Import",
                                                                "Test_ImportDoiTuong.json")

    @allure.testcase("c4i2-1468", "c4i2-1468")
    @allure.story("Import dữ liệu Đối tượng thành công")
    @allure.title("Xác minh import file thành công")
    @pff.parametrize(path=pathDataTestImport, key="test_ImportDoiTuongSuccess")
    def test_ImportFileObjDataSuccess(self, browser, Data, statusImport):
        """Trường hợp kiểm thử Import file dữ liệu đối tượng thành công - dữ liệu hợp lệ, đầy đủ thông tin"""
        driver = browser
        page_base = PageBase(driver)
        json_data_handler = JsonDataHandler(Data)
        page_import = PageImport(driver)

        page_base.show_overlay_text("Trường hợp kiểm thử file Import Đối tượng thành công")

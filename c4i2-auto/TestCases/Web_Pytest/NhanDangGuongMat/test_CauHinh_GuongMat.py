...
def browser():
    ...
    pageLogin = LoginPage(driver)
    pageBase = PageBase(driver)
    driver.get(url)
    pageLogin.do_login(username, password)
    pageLogin.click_obj(Obj_GuongMat.iconNhanDangGuongMat)
    pageBase.click_obj(Obj_GuongMat.tabCauHinh)
    time.sleep(1)
...
@allure.feature("Cấu hình Gương mặt")
@pytest.mark.FS
@pytest.mark.Add_DataList
class Test_FSCauHinh:
    pageBase = PageBase(browser)
    pathDataTestConfigFS = pageBase.load_path_data_file_from_path("Datas_FS",
                                                                  "Test_CauHinh_GuongMat.json")

    @pytest.mark.UAT_DC
    @allure.testcase("c4i2-767", "c4i2-767")
    @allure.title("c4i2-767: Xác minh có thể tạo loại theo dõi mới")
    @pff.parametrize(path=pathDataTestConfigFS)
    def test_creation_of_new_tracking_type(self, browser, Data, expected):
        """Trường hợp kiểm thử xác minh có thể tạo loại theo dõi mới cho gương mặt"""
        driver = browser
        ...
        with allure.step("Điều hướng đến 'Nhóm theo dõi'"):
            page_base.click_obj(ObjCommon.element_text("Nhóm theo dõi"))
            capture_screenshot_and_attach_allure(driver, "PageTracking")
        with allure.step("Nhấn vào button 'Thêm'"):
            page_base.click_obj(Obj_GuongMat.btnThemMoiLTD)
        ...
        if expected_status:
            ...
            with allure.step("Kiểm tra thông báo lỗi"):
                ...
                capture_screenshot_and_attach_allure(driver, name="MessageError")

    # @pytest.mark.depends(on=['test_creation_of_new_tracking_type'])
    @pytest.mark.run(after='test_creation_of_new_tracking_type')
    @pytest.mark.UAT_DC
    @allure.testcase("4i2-769", "c4i2-769")
    @allure.title("c4i2-769: Xác minh có thể xóa loại theo dõi")
    @pff.parametrize(path=pathDataTestConfigFS, key="test_creation_of_new_tracking_type")
    def test_delete_tracking_type(self, browser, Data, expected):
        ...
        if expected_status:
            ...
            with allure.step("Nhấn xóa"):
                ...
                time.sleep(1)
                page_base.click_obj(Obj_GuongMat.btnXacNhanXoa)
                time.sleep(1)
